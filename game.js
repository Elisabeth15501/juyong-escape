 ...
 *   - 台上 65% 概率放大将军印引路（Temple Run 2「金币位置=操作提示」）
 *   - 速度自适应台宽（130→300px，落台时机窗恒定 ~0.54s）+ 24px 落台吸附带
 *   - 张钦台上甩尾：追击中玩家上台 → 断锁放弃（他上不了台）
 * v1.2.0：千斤闸——四拍相位机 + 屏缘双感叹号预警 + 无限 60 里/幕 4/幕 7
 * v1.1.0：教学体系（幕1-4 + 无限头5障碍）+ 手感重做 + 辅助模式
 * v1.0.0：八幕扩关 + 关卡选择——
 ...
 *           幕 8 终局：居庸关门 → 张钦痛哭失声演出（变身可撞碎大门，隐藏成就）
 * 自适应布局（v0.4）：
...
function spawnObstacle() {
  ...
  if (mode === 'endless') {
    ...
    types = currentLevel().types.slice();                 // 张钦按幕配置在 LEVELS types 里（幕 4/幕 6/幕 8，与锁同规则）
  }
